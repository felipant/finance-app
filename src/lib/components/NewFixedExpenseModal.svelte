<script lang="ts">
	import { X, Save, Search } from 'lucide-svelte';
	
	let { onclose, onsave, onnotification } = $props();
	...
	async function handleSave() {
		...
		const newGasto = {
			...
			categoria: selectedItem?.nome_categoria || '',
			subcategoria: selectedItem?.nome_subcategoria || ''
		};
		
		await onsave(newGasto);
	}
...
<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
	<div class="bg-slate-900 border border-slate-700 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
		<div class="flex items-center justify-between p-6 border-b border-slate-700">
			<h2 class="text-2xl font-bold text-white">Novo Gasto Fixo</h2>
			<button
				onclick={onclose}
